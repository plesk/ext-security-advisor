...
class FileSystem
{
     ...
     * @param string $mode The file mode for newly created directories.
     * @param bool $recursive Allows the creation of nested directories specified in the pathname.
     * @throws \pm_Exception on failure.
     */
    public static function ensureDirExists($pathname, $mode = '0755', $recursive = true)
    {
        $fileManager = (new \pm_ServerFileManager());
        if (!$fileManager->fileExists($pathname) && !$fileManager->mkdir($pathname, $mode, $recursive)) {
